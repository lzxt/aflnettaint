...
EXP_ST u8  skip_deterministic,        /* Skip deterministic stages?       */
           ...
           llm_mode_enabled = 0,      /* LLM mutation mode enabled?      */
           llm_mutation_active = 0,   /* Currently in LLM mutation phase? */
           llm_triggered = 0,         /* LLM mutation has been triggered? */
           dataflow_guidance_enabled = 0, /* Data flow guidance enabled? */
           taint_analysis_enabled = 0; /* Taint analysis enabled? */
...
/* Data flow guidance: track which input bytes lead to new control flow edges */
static u8  dataflow_valid_bytes[MAX_FILE];  /* Valid data flow positions */
static u32 dataflow_valid_count = 0;        /* Count of valid positions */
static u8  prev_trace_bits[MAP_SIZE];        /* Previous execution trace */
static u8  dataflow_tracking_enabled = 0;   /* Runtime tracking enabled */

/* Taint analysis: Byte-to-CMP mapping */
#define MAX_CMP_ID 4096  /* Maximum number of CMP IDs */
...
#define ENERGY_WEIGHT_DF      130  /* 1.3 */
#define ENERGY_WEIGHT_PROTOCOL 140 /* 1.4 */
#define DF_ALPHA_PERCENT      150  /* alpha = 1.5 */
...
EXP_ST u64 total_crashes,             /* Total number of crashes          */
           ...
           execs_wo_new_paths = 0,        /* Execs without finding new paths  */
           execs_at_last_cov_check = 0,   /* Execs at last coverage check    */
           last_cov_check_time = 0,       /* Time of last coverage check      */
           llm_seed_count = 0,            /* Number of LLM-generated seeds   */
           llm_execs_remaining = 0,       /* Remaining LLM mutation execs     */
           coverage_at_50k = 0;           /* Coverage when reached 50k execs */
...
/* LLM-generated seed entry */
struct llm_seed_entry {
  u8* fname;                          /* File name for the test case      */
  u32 len;                            /* Input length                     */
  struct llm_seed_entry *next;        /* Next element, if any            */
};
...
struct queue_entry {
  ...
  u8  cal_failed,                     /* Calibration failed?              */
      ...
      df_interesting;                 /* Data flow interesting seed?     */
...
static struct llm_seed_entry *llm_seed_queue = NULL,  /* LLM-generated seed queue */
                             *llm_seed_cur = NULL;    /* Current LLM seed        */
...
u8 upnp_mode = 0; /* UPNP (SOAP-over-HTTP) helper tweaks */
...
static void upnp_fix_content_length_inplace(char* msg, unsigned int len) {
  /* Best-effort fix: rewrite Content-Length digits to match actual body size.
     Does not change message length; only overwrites existing digit run. */
  ...
  for (i = 0; i + 4 <= len; i++) {
    if (!memcmp(msg + i, term, 4)) { hdr_end = (int)i + 4; break; }
  }
  if (hdr_end < 0 || (unsigned int)hdr_end > len) return;

  unsigned int body_len = len - (unsigned int)hdr_end;

  /* Find Content-Length: (case-insensitive) at line starts */
  const char* key = "Content-Length:";
  const unsigned int key_len = 15;

  for (i = 0; i + key_len <= (unsigned int)hdr_end; i++) {
    if (i != 0) {
      if (i < 2) continue;
      if (!(msg[i - 2] == '\r' && msg[i - 1] == '\n')) continue;
    }

    /* case-insensitive compare */
    unsigned int j = 0;
    for (j = 0; j < key_len; j++) {
      char a = (char)tolower((unsigned char)msg[i + j]);
      char b = (char)tolower((unsigned char)key[j]);
      if (a != b) break;
    }
    if (j != key_len) continue;

    unsigned int p = i + key_len;
    while (p < (unsigned int)hdr_end && (msg[p] == ' ' || msg[p] == '\t')) p++;

    unsigned int dstart = p;
    while (p < (unsigned int)hdr_end && isdigit((unsigned char)msg[p])) p++;

    unsigned int width = p - dstart;
    if (width == 0) return;

    char tmp[32];
    snprintf(tmp, sizeof(tmp), "%u", body_len);
    unsigned int tlen = (unsigned int)strlen(tmp);
    if (tlen > width) return; /* cannot fit without resizing */

    /* zero-pad on the left to keep fixed width */
    unsigned int pad = width - tlen;
    unsigned int k = 0;
    for (k = 0; k < pad; k++) msg[dstart + k] = '0';
    memcpy(msg + dstart + pad, tmp, tlen);
    return;
  }
}
...
static struct queue_entry *choose_df_seed_from_state(state_info_t *state) {
  if (!taint_analysis_enabled || !state || !state->seeds_count) return NULL;
  ...
  for (u32 i = 0; i < state->seeds_count; i++) {
    ...
    if (!q || !q->df_interesting || !df_seed_is_high_quality(q)) continue;

    u32 score = 1000;
    score += (u32)q->df_new_cmp_cnt * 80;
    score += (u32)q->df_frontier_cmp_cnt * 150;
    score += (u32)q->df_density_pm;
    if (!q->was_fuzzed) score += 500;
    if (q->favored) score += 250;
    if (q->seed_class == SEED_CLASS_PROTOCOL_STATE_NEW) score += 300;

    if (score > best_score || (score == best_score && UR(2))) {
      best_score = score;
      best = q;
    }
  }
...
struct queue_entry *choose_seed(u32 target_state_id, u8 mode)
{
  ...
  if (k != kh_end(khms_states)) {
    ...
    if (taint_analysis_enabled) {
      u32 df_pick_prob = df_heating_mode ? 85 : 35;
      if (UR(100) < df_pick_prob) {
        result = choose_df_seed_from_state(state);
...
int send_over_network()
{
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    if (upnp_mode) upnp_fix_content_length_inplace(kl_val(it)->mdata, kl_val(it)->msize);
...
EXP_ST void setup_shm(void) {
  ...
  if (taint_analysis_enabled) {
    ...
    setenv("AFL_TAINT_ANALYSIS", "1", 1);
    
    taint_map_shm_id = shmget(IPC_PRIVATE, taint_map_size, IPC_CREAT | IPC_EXCL | 0600);
    
    if (taint_map_shm_id < 0) PFATAL("shmget() failed for taint map");
    
    taint_shm_str = alloc_printf("%d", taint_map_shm_id);
    setenv("__AFL_TAINT_MAP_SHM_ID", taint_shm_str, 1);
...
static void read_testcases(void) {
  ...
  /* Safety check: in_dir must be set */
  if (!in_dir) {
    FATAL("Input directory not set. This should not happen.");
  }
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  /* Clear taint map before each execution */
...
static void collect_taint_feedback(u8 hnb, const char *reason,
                                   u8 consume_frontier, u8 write_stats) {

  if (!taint_analysis_enabled || !taint_map_shm) return;
  ...
  u32 iter_cmp_count = active_cmp_count ? active_cmp_count : MAX_CMP_ID;
  ...
  if (prev_taint_map) {
    ...
    for (u32 idx = 0; idx < iter_cmp_count; idx++) {
      u32 cmp_id = active_cmp_count ? active_cmp_ids_shm[idx + 1] : idx;
    ...
    if (cmp_hit_shm && cmp_frontier_global) {
      for (u32 idx = 0; idx < iter_cmp_count; idx++) {
        u32 cid = active_cmp_count ? active_cmp_ids_shm[idx + 1] : idx;
  ...
  if (consume_frontier) consume_frontier_cmps_from_run(cmp_has_new_df);

  if (write_stats) {
    const char *r = reason;
    if (!r) {
      if (!cmp_triggered) r = "no_tainted_cmp";
      else if (!df_change) r = "no_new_df";
      else if (!new_df_cmp) r = "no_new_df_cmp";
      else r = "df_feedback";
    }
    write_taint_stats(hnb, active_cmp_count, cmp_triggered, df_change,
                      total_cmp_tainted, new_df_cmp, frontier_new_cmp, 0, r);
  }
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    if (!(hnb = has_new_bits(virgin_bits))) {
      ...
      /* Taint analysis: Check for data flow changes even without new edges */
      if (taint_analysis_enabled && taint_map_shm) {
        ...
        u32 frontier_new_cmp = 0;
        u8 saved_df = 0;
        static u32 df_tainted_fallback_saved = 0;
        ...
        if (prev_taint_map) {
          ...
          if (prev_taint_map) {
            memset(prev_taint_map, 0, TAINT_MAP_SIZE);
            df_change = 1;  /* First seed is interesting */
          }
        }
        if (cmp_triggered && !total_cmp_tainted && stage_cur_byte >= 0 &&
            (u32)stage_cur_byte < len) {
          total_cmp_tainted = 1;
          df_change = 1;
          new_df_cmp = 1;
        }
        
        if (cmp_triggered && total_cmp_tainted > 0) {
          /* Frontier-aware check removed: any new DF on active CMPs is valuable. */
          /* (frontier_new_cmp is kept for stats compatibility but no longer gates saving.) */
          if (cmp_hit_shm && cmp_frontier_global) {
            for (u32 idx = 0; idx < iter_cmp_count; idx++) {
              u32 cid = active_cmp_count ? active_cmp_ids_shm[idx + 1] : idx;
              if (cid >= MAX_CMP_ID) continue;
          ...
          u8 leads_to_uncovered = new_df_cmp > 0;
          u8 tainted_behavior_seed = (!leads_to_uncovered && total_cmp_tainted > 0 &&
                                      df_tainted_fallback_saved < 4096 &&
                                      (queue_cycle <= 2 || UR(100) < 15));
          if (!cmp_triggered) df_skip_reason = "no_tainted_cmp";
          else if (!total_cmp_tainted) df_skip_reason = "no_taint_bytes";
          else if (leads_to_uncovered) df_skip_reason = "save_df_new";
          else if (tainted_behavior_seed) df_skip_reason = "save_df_tainted";
          else if (!df_change) df_skip_reason = "no_new_df";
          else if (!new_df_cmp) df_skip_reason = "no_new_df_cmp";
          else df_skip_reason = "save_df";
          
          /* 如果数据流变化或已有 tainted CMP 行为关联，标记为 DF_Interesting */
          if (leads_to_uncovered || tainted_behavior_seed) {
            saved_df = 1;
            df_interesting_seed = 1;
            /* Still save to queue for directed mutation */
            fn = alloc_printf("%s/queue/id:%06u,s:df,src:%06u,op:%s,+df", out_dir,
                              queued_paths, current_entry, stage_short ? stage_short : "unknown");
            ...
            if (queue_top) {
              ...
              queue_top->df_density_pm = (queue_top->df_total_cmp_cnt == 0) ? 0 :
                (u16)((1000u * (queue_top->df_new_cmp_cnt ? queue_top->df_new_cmp_cnt : 1)) / queue_top->df_total_cmp_cnt);
              ...
              if (max_off > 0) {
                ...
                for (u32 idx = 0; idx < iter_cmp_count; idx++) {
                  u32 cid = active_cmp_count ? active_cmp_ids_shm[idx + 1] : idx;
                  if (cid >= MAX_CMP_ID) continue;
                  if (cmp_hit_shm && !cmp_hit_shm[cid]) continue;
                  if (!tainted_behavior_seed && !cmp_has_new_df[cid]) continue;
                ...
                if (!queue_top->df_key_count && stage_cur_byte >= 0 &&
                    (u32)stage_cur_byte < max_off) {
                  queue_top->df_key_offsets = ck_alloc(sizeof(u32));
                  queue_top->df_key_offsets[0] = (u32)stage_cur_byte;
                  queue_top->df_key_count = 1;
                  queue_top->df_min_offset = (u32)stage_cur_byte;
                  queue_top->df_max_offset = (u32)stage_cur_byte;
                  queue_top->df_focus_pm = (full_len == 0) ? 0 :
                    (u16)MIN(1000u, 1000u / full_len);
                }
                ck_free(key_scores);
              }
            }
            ...
            if (prev_taint_map) {
              u32 update_cmp_count = active_cmp_count ? active_cmp_count : MAX_CMP_ID;
              for (u32 idx = 0; idx < update_cmp_count; idx++) {
                u32 cmp_id = active_cmp_count ? active_cmp_ids_shm[idx + 1] : idx;
                if (cmp_id >= MAX_CMP_ID) continue;
                if (!cmp_has_new_df[cmp_id]) continue;
            ...
            if (tainted_behavior_seed) df_tainted_fallback_saved++;
            write_taint_stats(hnb, active_cmp_count, cmp_triggered, df_change,
      ...
      /* Update execs without new paths counter */
      if (llm_mode_enabled && !llm_mutation_active) {
        execs_wo_new_paths++;
      }
      return 0;
    }
    ...
    if (taint_analysis_enabled) {
      if (taint_map_shm) {
        collect_taint_feedback(hnb, "new_coverage_cf_seed", 0, 1);
      }
    ...
    if (llm_mode_enabled) {
      execs_wo_new_paths = 0;
      last_cov_check_time = get_cur_time();
      /* Update coverage_at_50k if we just reached 50k */
      if (total_execs >= 50000 && coverage_at_50k == 0) {
        coverage_at_50k = (MAP_SIZE << 3) - count_bits(virgin_bits);
      }
    }
...
EXP_ST void check_binary(u8* fname) {
  ...
  if (taint_analysis_enabled &&
      !memmem(f_data, f_len, "__AFL_TAINT_MAP_SHM_ID", 22) &&
      !memmem(f_data, f_len, "__afl_check_taint", 17) &&
      !memmem(f_data, f_len, "__afl_taint_source", 18)) {

    SAYF("\n" cLRD "[-] " cRST
         "Taint mode is enabled, but the target binary does not contain taint\n"
         "    instrumentation/runtime symbols. Rebuild the target with:\n\n"
         "    AFL_TAINT_ANALYSIS=1 CC=/path/to/afl-clang-fast ./configure ...\n"
         "    AFL_TAINT_ANALYSIS=1 make clean all\n\n"
         "    Then run afl-fuzz with -T taint.\n\n");

    FATAL("No taint instrumentation detected");
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -P protocol   - application protocol to be tested (e.g., RTSP, FTP, DTLS12, DNS, SMTP, SSH, TLS, HTTP, UPNP)\n"
...
/* Generate initial seed using RAG if no input directory is provided */
static void generate_seed_with_rag_if_needed(void) {
  
  if (in_dir) return; /* Input directory already provided */
  ...
  ACTF("No input directory specified. Generating initial seed using RAG...");
  
  /* Create temporary input directory */
  temp_input_dir = alloc_printf("%s/.rag_generated_input", out_dir);
  if (mkdir(temp_input_dir, 0700) && errno != EEXIST) {
    PFATAL("Unable to create temporary input directory '%s'", temp_input_dir);
  }
  
  /* Find the RAG script path - try multiple possible locations */
  script_path = NULL;
  
  /* Try 1: Use AFL_PATH environment variable if set (most reliable) */
  if (getenv("AFL_PATH")) {
    script_path = alloc_printf("%s/tutorials/upnp/generate_seed_with_rag.py", getenv("AFL_PATH"));
  ...
  /* Try 2: Use DOC_PATH (typically /usr/local/share/doc/afl or similar) */
  /* Go up from doc_path to find tutorials directory */
  if (doc_path) {
    u8* doc_parent = alloc_printf("%s/..", doc_path);
    script_path = alloc_printf("%s/tutorials/upnp/generate_seed_with_rag.py", doc_parent);
    if (access(script_path, F_OK) == 0) {
      ck_free(doc_parent);
      goto script_found;
    }
  ...
  /* Try 3: tutorials/upnp/generate_seed_with_rag.py (relative to current dir) */
  script_path = alloc_printf("tutorials/upnp/generate_seed_with_rag.py");
  if (access(script_path, F_OK) == 0) {
    goto script_found;
  }
  ck_free(script_path);
  
  /* Try 4: ./tutorials/upnp/generate_seed_with_rag.py */
  script_path = alloc_printf("./tutorials/upnp/generate_seed_with_rag.py");
  if (access(script_path, F_OK) == 0) {
    goto script_found;
  }
  ...
  /* Try 5: Try to find it in parent directories */
  script_path = alloc_printf("../tutorials/upnp/generate_seed_with_rag.py");
  if (access(script_path, F_OK) == 0) {
    goto script_found;
  }
  ...
  /* Try 6: ../../tutorials/upnp/generate_seed_with_rag.py */
  script_path = alloc_printf("../../tutorials/upnp/generate_seed_with_rag.py");
  if (access(script_path, F_OK) == 0) {
    goto script_found;
  }
  ...
  /* Script not found - provide helpful error message */
  ck_free(temp_input_dir);
  SAYF("\n" cLRD "[-] " cRST
       "Could not find generate_seed_with_rag.py script.\n"
       "    Tried the following locations:\n");
  if (getenv("AFL_PATH")) {
    SAYF("      - $AFL_PATH/tutorials/upnp/generate_seed_with_rag.py\n");
  }
  SAYF("      - tutorials/upnp/generate_seed_with_rag.py\n"
       "      - ./tutorials/upnp/generate_seed_with_rag.py\n"
       "      - ../tutorials/upnp/generate_seed_with_rag.py\n"
       "\n"
       "    Solutions:\n"
       "      1. Set AFL_PATH environment variable to your AFLNet source directory:\n"
       "         export AFL_PATH=/path/to/aflnet\n"
       "      2. Use -i option to specify an input directory with test cases\n"
       "      3. Copy generate_seed_with_rag.py to one of the above locations\n");
  FATAL("RAG script not found");
  
script_found:
  /* Script found, continue */
  
  /* Build command to execute Python script - try python3 first, then python */
  cmd = alloc_printf("python3 \"%s\" \"%s\" 2>&1", script_path, temp_input_dir);
  
  ACTF("Executing: %s", cmd);
  ret = system(cmd);
  
  /* If python3 failed, try python (for Windows compatibility) */
  if (ret != 0) {
    ck_free(cmd);
    cmd = alloc_printf("python \"%s\" \"%s\" 2>&1", script_path, temp_input_dir);
    ACTF("Trying with python instead: %s", cmd);
    ret = system(cmd);
  }
  ...
  if (ret != 0) {
    SAYF("\n" cLRD "[-] " cRST
         "Failed to generate initial seed using RAG. Exit code: %d\n"
         "    You can manually specify an input directory using -i option.\n", ret);
    ck_free(temp_input_dir);
    FATAL("RAG seed generation failed");
  }
  
  /* Set in_dir to the generated directory */
  in_dir = temp_input_dir;
  
  ACTF("Initial seed generated successfully in '%s'", in_dir);
}

/* Check if LLM mutation should be triggered */
static u8 check_llm_trigger_conditions(void) {
  /* Early return if LLM mode not enabled or already active */
  if (!llm_mode_enabled || llm_mutation_active) return 0;
  
  u64 cur_time = get_cur_time();
  u32 cur_coverage = (MAP_SIZE << 3) - count_bits(virgin_bits);
  
  /* Condition 1: 连续 10000 次测试未发现新路径 */
  if (execs_wo_new_paths >= 10000) {
    ACTF("LLM trigger: 10000 execs without new paths");
    return 1;
  }
  
  /* Condition 2: 累计执行 50000 次后覆盖率增长 < 1% */
  if (total_execs >= 50000) {
    if (coverage_at_50k == 0) {
      /* First time reaching 50k, record coverage and don't trigger yet */
      coverage_at_50k = cur_coverage;
      execs_at_last_cov_check = total_execs;
      return 0;
    } else {
      /* Check if growth is less than 1% */
      if (coverage_at_50k > 0 && cur_coverage > coverage_at_50k) {
        double cov_growth = ((double)(cur_coverage - coverage_at_50k)) / coverage_at_50k * 100.0;
        if (cov_growth < 1.0) {
          ACTF("LLM trigger: Coverage growth < 1%% after 50000 execs (%.2f%%)", cov_growth);
          return 1;
        }
      } else if (coverage_at_50k > 0 && cur_coverage <= coverage_at_50k) {
        /* Coverage hasn't increased at all */
        ACTF("LLM trigger: No coverage growth after 50000 execs");
        return 1;
      }
    }
  }
  
  /* Condition 3: 连续 30 分钟无新覆盖率 */
  if (last_cov_check_time > 0 && (cur_time - last_cov_check_time) >= 30 * 60 * 1000) {
    ACTF("LLM trigger: No new coverage for 30 minutes");
    return 1;
  }
...
/* Find RAG mutation script */
static u8* find_mutation_script(void) {
  u8* script_path;
  
  script_path = alloc_printf("tutorials/upnp/mutate_with_llm_rag.py");
  if (access(script_path, F_OK) == 0) return script_path;
  ck_free(script_path);
  ...
  if (getenv("AFL_PATH")) {
    script_path = alloc_printf("%s/tutorials/upnp/mutate_with_llm_rag.py", getenv("AFL_PATH"));
    if (access(script_path, F_OK) == 0) return script_path;
    ck_free(script_path);
  }
...
/* Mutate existing seeds using LLM and RAG */
static void mutate_seeds_with_llm(u8* seed_file, u8* output_dir, u32 num_mutations) {
  u8* script_path;
  u8* cmd;
  s32 ret;
  
  script_path = find_mutation_script();
  if (!script_path) {
    ACTF("Could not find mutation script mutate_with_llm_rag.py");
    return;
  }
  
  cmd = alloc_printf("python3 \"%s\" \"%s\" \"%s\" %u 2>&1", 
                     script_path, seed_file, output_dir, num_mutations);
  ret = system(cmd);
  if (ret != 0) {
    ck_free(cmd);
    cmd = alloc_printf("python \"%s\" \"%s\" \"%s\" %u 2>&1", 
                       script_path, seed_file, output_dir, num_mutations);
    ret = system(cmd);
  }
  ...
  if (ret != 0) {
    ACTF("LLM mutation failed, exit code: %d", ret);
  }
}

/* Generate LLM seeds using RAG - includes both new generation and mutation */
static void generate_llm_seeds(void) {
  u8* script_path;
  u8* temp_llm_dir;
  u8* cmd;
  s32 ret;
  ...
  u32 i;
  struct queue_entry *q;
  u32 selected_seeds = 0;
  u32 max_seeds_to_mutate = 5; /* Mutate up to 5 existing seeds */
  
  ACTF("Generating LLM seeds using RAG (generation + mutation)...");
  
  /* Create temporary directory for LLM seeds */
  temp_llm_dir = alloc_printf("%s/.llm_seeds", out_dir);
  if (mkdir(temp_llm_dir, 0700) && errno != EEXIST) {
    PFATAL("Unable to create LLM seed directory '%s'", temp_llm_dir);
  }
  
  /* Step 1: Generate new seeds from scratch */
  script_path = alloc_printf("tutorials/upnp/generate_seed_with_rag.py");
  if (access(script_path, F_OK) != 0) {
    ck_free(script_path);
    script_path = alloc_printf("./tutorials/upnp/generate_seed_with_rag.py");
  ...
  if (script_path) {
    cmd = alloc_printf("python3 \"%s\" \"%s\" 2>&1", script_path, temp_llm_dir);
    ret = system(cmd);
    if (ret != 0) {
      ck_free(cmd);
      cmd = alloc_printf("python \"%s\" \"%s\" 2>&1", script_path, temp_llm_dir);
      ret = system(cmd);
    }
    ck_free(cmd);
    ck_free(script_path);
  ...
  /* Step 2: Mutate existing seeds from queue */
  /* Select some interesting seeds from the queue for mutation */
  q = queue;
  if (!q) {
    ACTF("No seeds in queue for mutation, only generating new seeds");
  }
  while (q && selected_seeds < max_seeds_to_mutate) {
    /* Prefer seeds that found new coverage or are favored */
    if (q->has_new_cov || q->favored || UR(100) < 30) {
      u8* mutation_output = alloc_printf("%s/.llm_mutations_%u", temp_llm_dir, selected_seeds);
      if (mkdir(mutation_output, 0700) && errno != EEXIST) {
        ck_free(mutation_output);
        q = q->next;
        continue;
      }
      
      ACTF("Mutating seed %s using LLM+RAG...", q->fname);
      mutate_seeds_with_llm(q->fname, mutation_output, 3); /* Generate 3 mutations per seed */
  ...
  /* Load all generated seeds (both new and mutated) into LLM queue */
  /* Helper function to load seeds from a directory - defined before generate_llm_seeds */
  /* We'll inline it here to avoid nested function issues */
  
  /* Load seeds from main directory */
  nl_cnt = scandir(temp_llm_dir, &nl, NULL, alphasort);
  if (nl_cnt >= 0) {
    for (i = 0; i < nl_cnt; i++) {
      struct stat st;
      u8* fn = alloc_printf("%s/%s", temp_llm_dir, nl[i]->d_name);
      ...
      if (S_ISDIR(st.st_mode)) {
        /* This is a mutation subdirectory, scan it */
        if (strstr(nl[i]->d_name, ".llm_mutations_")) {
          struct dirent **sub_nl;
          s32 sub_nl_cnt;
          u32 j;
          
          sub_nl_cnt = scandir(fn, &sub_nl, NULL, alphasort);
          if (sub_nl_cnt >= 0) {
            for (j = 0; j < sub_nl_cnt; j++) {
              struct stat sub_st;
              u8* sub_fn = alloc_printf("%s/%s", fn, sub_nl[j]->d_name);
              
              if (lstat(sub_fn, &sub_st) == 0 && S_ISREG(sub_st.st_mode) && sub_st.st_size > 0) {
                if (strstr(sub_fn, ".raw") || strstr(sub_fn, "mutated_seed")) {
                  struct llm_seed_entry *llm_seed = ck_alloc(sizeof(struct llm_seed_entry));
                  llm_seed->fname = sub_fn;
                  llm_seed->len = sub_st.st_size;
                  llm_seed->next = NULL;
                  
                  if (llm_seed_queue) {
                    struct llm_seed_entry *tmp = llm_seed_queue;
                    while (tmp->next) tmp = tmp->next;
                    tmp->next = llm_seed;
                  } else {
                    llm_seed_queue = llm_seed;
                    llm_seed_cur = llm_seed;
                  }
        ...
        if (strstr(fn, ".raw") || strstr(fn, "seed")) {
          struct llm_seed_entry *llm_seed = ck_alloc(sizeof(struct llm_seed_entry));
          llm_seed->fname = fn;
          llm_seed->len = st.st_size;
          llm_seed->next = NULL;
  ...
  if (llm_seed_count == 0) {
    ACTF("No LLM seeds generated");
    ck_free(temp_llm_dir);
    return;
  }
  
  ACTF("Loaded %llu LLM-generated seeds (%u seeds mutated)", llm_seed_count, selected_seeds);
}
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:e:P:KEq:s:RFc:l:b:h:L")) > 0)
    ...
    switch (opt) {
      ...
      case 'T': /* taint analysis, data flow guidance, or banner */
        
        if (taint_analysis_enabled || dataflow_guidance_enabled || use_banner) 
          FATAL("Multiple -T options not supported");
        
        /* If optarg is "taint", enable taint analysis */
        if (optarg && !strcmp(optarg, "taint")) {
          taint_analysis_enabled = 1;
        } else if (!optarg || !optarg[0] || !strcmp(optarg, "dataflow")) {
          /* If optarg is "dataflow" or empty, enable data flow guidance */
          dataflow_guidance_enabled = 1;
          dataflow_tracking_enabled = 1;
        } else {
          /* Otherwise, set banner (backward compatibility) */
          use_banner = optarg;
        }
        break;
      ...
      case 'L': /* LLM mutation mode */
        if (llm_mode_enabled) FATAL("Multiple -L options not supported");
        llm_mode_enabled = 1;
        break;
  ...
  /* If no input directory specified, we'll generate it using RAG later */
  /* Don't require in_dir here, we'll handle it in setup_dirs_fds */
  ...
  if (in_dir && !strcmp(in_dir, out_dir))
  ...
  /* Generate initial seed with RAG if no input directory was provided */
  /* This must be called before read_testcases() */
  if (!in_dir) {
    generate_seed_with_rag_if_needed();
  }
  
  /* Now in_dir should be set (either from -i or from RAG generation) */
  if (!in_dir) {
    FATAL("No input directory specified and RAG seed generation failed. Please use -i option.");
  }
  ...
  /* Initialize LLM mode variables */
  if (llm_mode_enabled) {
    llm_mutation_active = 0;
    llm_triggered = 0;
    execs_wo_new_paths = 0;
    execs_at_last_cov_check = 0;
    last_cov_check_time = get_cur_time();
    coverage_at_50k = 0;
    llm_seed_count = 0;
    llm_execs_remaining = 0;
    llm_seed_queue = NULL;
    llm_seed_cur = NULL;
    ACTF("LLM mutation mode enabled");
  }
  ...
  if (seed_schedule_type == HYBRID_SCHEDULE) {
    ...
    while (1) {
      ...
      if (llm_mode_enabled && !llm_mutation_active && check_llm_trigger_conditions()) {
        ACTF("LLM mutation triggered! Generating seeds...");
        llm_triggered = 1;
        llm_mutation_active = 1;
        llm_execs_remaining = 1000;
        generate_llm_seeds();
        if (llm_seed_count == 0) {
          ACTF("No LLM seeds generated, continuing normal fuzzing");
          llm_mutation_active = 0;
        }
      }

      /* LLM mutation phase: use LLM-generated seeds */
      if (llm_mutation_active && llm_seed_cur && llm_execs_remaining > 0) {
        /* Load LLM seed and fuzz it */
        FILE *fp = fopen(llm_seed_cur->fname, "rb");
        if (fp) {
          u8 *llm_buf = ck_alloc(llm_seed_cur->len);
          size_t bytes_read = fread(llm_buf, 1, llm_seed_cur->len, fp);
          fclose(fp);
          
          if (bytes_read == llm_seed_cur->len) {
            /* Test the LLM seed - need to run target first */
            write_to_testcase(llm_buf, llm_seed_cur->len);
            u8 fault = run_target(use_argv, exec_tmout);
            
            /* Update execution counters */
            total_execs++;
            
            /* Check if interesting and save */
            u8 had_new_path = save_if_interesting(use_argv, llm_buf, llm_seed_cur->len, fault);
            
            if (!had_new_path && llm_mode_enabled) {
              execs_wo_new_paths++;
            }
          }
          ...
          /* Move to next LLM seed */
          llm_seed_cur = llm_seed_cur->next;
          if (!llm_seed_cur) {
            llm_seed_cur = llm_seed_queue; /* Cycle back */
          }
          
          if (llm_execs_remaining == 0) {
            ACTF("LLM mutation phase completed, returning to normal fuzzing");
            llm_mutation_active = 0;
            execs_wo_new_paths = 0; /* Reset counter */
            last_cov_check_time = get_cur_time();
            /* Update coverage_at_50k if we just reached 50k */
            if (total_execs >= 50000 && coverage_at_50k == 0) {
              coverage_at_50k = (MAP_SIZE << 3) - count_bits(virgin_bits);
          ...
          continue; /* Skip normal fuzzing during LLM phase */
        } else {
          /* File not found, skip to next */
          llm_execs_remaining--;
          llm_seed_cur = llm_seed_cur->next;
          if (!llm_seed_cur) {
            llm_seed_cur = llm_seed_queue;
            if (!llm_seed_cur) {
              llm_execs_remaining = 0;
              llm_mutation_active = 0;
            }
