...
static int memcasecmp_local(const unsigned char* a, const unsigned char* b, unsigned int n) {
  unsigned int i = 0;
  for (i = 0; i < n; i++) {
    unsigned char ca = (unsigned char)tolower(a[i]);
    unsigned char cb = (unsigned char)tolower(b[i]);
    if (ca != cb) return (int)ca - (int)cb;
  }
  return 0;
}

static int find_subseq_local(const unsigned char* buf, unsigned int buf_size,
                             unsigned int start, const unsigned char* pat, unsigned int pat_len) {
  unsigned int i = 0;
  if (pat_len == 0 || buf_size < pat_len || start >= buf_size) return -1;
  for (i = start; i + pat_len <= buf_size; i++) {
    if (!memcmp(buf + i, pat, pat_len)) return (int)i;
  }
  return -1;
}

static int parse_content_length_local(const unsigned char* hdr, unsigned int hdr_len, unsigned int* out_len) {
  /* Parse "Content-Length:" in HTTP headers (case-insensitive). */
  static const unsigned char key[] = "Content-Length:";
  unsigned int i = 0;
  if (!hdr || hdr_len == 0 || !out_len) return 0;

  for (i = 0; i + (unsigned int)(sizeof(key) - 1) <= hdr_len; i++) {
    /* line start: beginning or after CRLF */
    if (i != 0) {
      if (i < 2) continue;
      if (!(hdr[i - 2] == '\r' && hdr[i - 1] == '\n')) continue;
    }

    if (memcasecmp_local(hdr + i, key, (unsigned int)(sizeof(key) - 1)) == 0) {
      unsigned int j = i + (unsigned int)(sizeof(key) - 1);
      while (j < hdr_len && (hdr[j] == ' ' || hdr[j] == '\t')) j++;

      unsigned int val = 0;
      unsigned int seen = 0;
      while (j < hdr_len && isdigit(hdr[j])) {
        seen = 1;
        /* avoid overflow */
        if (val > 429496729U) break;
        val = val * 10U + (unsigned int)(hdr[j] - '0');
        j++;
      }
      if (seen) {
        *out_len = val;
...
/*
 * UPNP control plane is SOAP-over-HTTP. Requests are typically HTTP POST with a body.
 * This extractor keeps each HTTP request as a single region: header (\r\n\r\n) + body (Content-Length bytes).
 */
region_t* extract_requests_upnp(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  unsigned int region_count = 0;
  region_t *regions = NULL;
  unsigned int pos = 0;
  static const unsigned char terminator[4] = {0x0D, 0x0A, 0x0D, 0x0A};

  while (pos < buf_size) {
    int hdr_start = (int)pos;
    int hdr_idx = find_subseq_local(buf, buf_size, pos, terminator, 4);

    if (hdr_idx < 0) {
      /* Remaining bytes: keep as last region */
      region_count++;
      regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
      regions[region_count - 1].start_byte = (int)pos;
      regions[region_count - 1].end_byte = (int)buf_size - 1;
      regions[region_count - 1].state_sequence = NULL;
      regions[region_count - 1].state_count = 0;
      break;
    }

    unsigned int hdr_end_excl = (unsigned int)hdr_idx + 4; /* points to first byte after \r\n\r\n */
    unsigned int content_len = 0;
    (void)parse_content_length_local(buf + hdr_start, hdr_end_excl - (unsigned int)hdr_start, &content_len);

    unsigned int msg_end_excl = hdr_end_excl + content_len;
    if (msg_end_excl > buf_size) msg_end_excl = buf_size;

    region_count++;
    regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
    regions[region_count - 1].start_byte = (int)pos;
    regions[region_count - 1].end_byte = (int)msg_end_excl - 1;
    regions[region_count - 1].state_sequence = NULL;
    regions[region_count - 1].state_count = 0;
...
unsigned int* extract_response_codes_upnp(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  /* For UPnP we reuse HTTP response codes as state feedback. */
  return extract_response_codes_http(buf, buf_size, state_count_ref);
}
